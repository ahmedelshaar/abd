...
<div class="modal fade" id="modal-fadein" tabindex="-1" aria-labelledby="modal-fadein" style="display: none;" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form method="post" id="delete-form" action="{{ route('social.destroy', 1)}}">
            {{ csrf_field() }}
            {{ method_field('DELETE') }}
    ...
    <div class="block-content">
        @if(count($data) > 0)
            ...
            @foreach($data as $type)
                ...
                <td><a href="{{$type->link}}" target="_blank">{{$type->link}}</a></td>
                <td class="text-center">
                    <div class="btn-group">
                        <a href="{{route('social.edit', $type)}}" class="btn btn-sm btn-secondary js-tooltip-enabled">
