...
<div class="modal fade" id="modal-fadein" tabindex="-1" aria-labelledby="modal-fadein" style="display: none;" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form method="post" id="delete-form" action="{{ route('offer.destroy', 1)}}">
            {{ csrf_field() }}
            {{ method_field('DELETE') }}
...
<div class="block">
    <div class="block-header block-header-default">
        <h3 class="block-title">Offers</h3>
    </div>
    <div class="block-content">
        @if(count($data) > 0)
            ...
            @foreach($data as $type)
                ...
                <td>{{$type->title}}</td>
                <td><img height="150px" src="{{asset($type->icon)}}" /></td>
                <td class="text-center">
                    <div class="btn-group">
                        <a href="{{route('offer.edit', $type)}}" class="btn btn-sm btn-secondary js-tooltip-enabled">
